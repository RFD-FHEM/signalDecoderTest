...
class BitStore
{
    public:
        ...
        unsigned char getValue(uint16_t pos);
...
template<uint8_t bufSize>
BitStore<bufSize>::BitStore(const uint8_t bitlength):buffsize(bufSize)
{
...
void BitStore<bufSize>::addValue(char value)
{
	...
	*/
    valcount++;
    if ((bcnt-valuelen) >= 0)  // Soalnge nicht 8 Bit gespeichert wurden, erhöhen wir den counter zum verschieben
    {
...
template<uint8_t bufSize>
unsigned char BitStore<bufSize>::getValue(uint16_t pos)
{
